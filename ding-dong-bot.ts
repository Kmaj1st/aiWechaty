...
function toggleModeCGPT(modeCGPT) {
  return !modeCGPT;
}
...
async function onMessage (msg: Message) {
  ...
  msg.say("mode:"+mode+" preMode:"+preMode)
...
async function sendMessage(msg){
  ...
  pyshell = new PythonShell('CGPT.py'); log.info("CGPT");
    ...
    if (response!=null){
      ...
      if(modeCGPT==true){
        modeCGPT = toggleModeCGPT(modeCGPT);
        try {
        msg.say(response)
        return;
        }
        ...
        msg.say(response.replace(/\r?/g,'').replace(/[“”"。，？?！!]|您/g, '').replace(/[!！]/g, '啊').replace('A:',''));
